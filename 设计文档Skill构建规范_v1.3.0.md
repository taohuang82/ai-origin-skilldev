...
| **新增约束** | Phase 6 补充"响应硬终止"语义声明；orchestration 要素循环补充"显式挂起"声明；SKILL.md 全局约束补充"操作菜单终止规则" | 3.1.1、3.2.2、3.4.1 |

**向后兼容性**: v1.2.0 新增字段均为可选，现有 v1.1.0 兼容的 Skill 在不使用新功能时无需修改即可运行。v1.2.1 的变更为强制约束补全，所有 Skill 必须同步更新 engine/*.md、orchestration/*.md、SKILL.md 中的对应位置。

---

## 版本变更摘要（v1.2.1 → v1.3.0）

| 变更分类 | 具体内容 | 影响章节 |
|---|---|---|
| **数据结构重构** | ImpactPoint 废弃 `kind` 字段，统一为单一结构；`boundary_constraints` 作为可选子字段嵌入，取代独立的 `IP-xxx-forbid` 条目 | 3.7.3 |
| **新增字段** | ImpactPoint 新增 `source_requirement`、`trigger_type`（已有）、`cascade_rule`（已有）、`target_state_evidence`、`out_of_scope_reason` 字段 | 3.7.3 |
| **新增 Schema** | 增量模式新增 RR（原始需求）运行时 Schema 定义，并在 `impact_analysis` 中增加 `requirement_register` 字段 | 3.7.3 |
| **新增 Schema** | §3.5.3 Step 1 新增 AtomicChange 运行时实例 Schema（区别于注册表静态条目） | 3.5.3 |
| **编排模式扩展** | §3.4.1 编排模板新增 Phase 2.5"全局收口"可选阶段，承载需要全局 ImpactPoint 收齐后才能执行的要素 | 3.4.1 |
| **evidence_source 规范化** | 明确 `evidence_source` / `target_state_evidence` 为 Skill 扩展字段，规范不固定枚举值 | 3.7.3 |

**向后兼容性**：v1.3.0 的 ImpactPoint `kind` 字段废弃属于破坏性变更，已有使用 `kind` 字段的 incremental 输出文档需做一次性迁移（搜索 `kind: "modify"` 和 `kind: "forbid"` 并按新结构重整）。其余新增字段均为可选，不影响既有文档读取。
...
└── workspace/                       # 运行时工作区（非 Skill 源码）
    ├── ongoing.md                   # 项目级全局状态锚点（schema 见  识别关键词![[ia-README]]   | ----- | ----------- | ------------------- |-dir}/                # 输出文档目录
    │   └─     {version}/
    │       └── {output-filen   me}.md
...
- {约束项2}
...
（约束项必须包含：引擎调用约束、状态写入约束、语言/风格约束、操作菜单终止规则）
...
# optional_input_folder_base: "workspace/..."     # 可选输入路径
...
⚠️ **响应硬终止规则（v1.2.1 新增，不可跳过）**：
输出上述操作菜单后，本次响应必须立即终止。
禁止在同一响应中推断用户意图、预判用户选择、继续执行下一要素或做任何后续动作。
必须等待用户发送下一条消息，由用户消息中的明确选择（C/B/S/Q）驱动后续行为。
本规则与 Phase 4 的 `[交互]` 步骤语义完全等价，属于"必须等待用户响应"的同类约束。
...
**Step 1 产出：AtomicChange 运行时实例**

每个识别出的变化点生成一个运行时实例（区别于 `atomic-change-registry.yaml` 的静态注册条目，后者只存目录元数据）：

```yaml
# AtomicChange 运行时实例 Schema（v1.3.0）
AtomicChange:
  id:                 "{CATEGORY-NN}"    # 引用 atomic-change-registry 中的变化点 ID，如 UI-01
  source_requirement: "RR-{xx}"         # 触发本变化点的原始需求编号
  evidence:           "证据原文片段"     # 识别依据的直接引用（原文，不得改写）
  evidence_source:    "..."             # 证据来源档位；具体枚举值由各 Skill 方案文档声明
  confidence:         "high | medium | low"
  open_question:      ""               # 置信度非 high 时的待确认问题；high 时置空字符串
```
...
- `registry/atomic-change-registry.yaml` 和 `change-element-mapping.yaml`（modify/incremental 模式）
- `registry/dependency-graph.yaml`（modify/incremental 模式作为安全网）
