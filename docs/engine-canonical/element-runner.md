...
本文件是**被 orchestration 派发的统一要素执行模板**，完全业务无感知。所有要素（无论何种 Skill、何种场景）都由本模板执行——**一个模板、N 参数、无 designer 文件**。要素之间的差异 100% 来自：spec（怎么做）+ standards（写成啥样）+ 派发参数（读哪个 spec、写哪个 artifact、要不要交互）。
...
- 本模板既可被主 agent **inline 直接执行**、也可被派发为 **subAgent 执行**，两种方式 Step 0–5 完全一致；dispatch 由编排层据配置决定，交互硬规则在两种方式下都必须生效。
