...
description: |
  Architecture Subagent — 产出 architecture.md
domain: architecture
...
|------|-----|
| TDD 章节 | 第1章 顶层架构 |
| 落盘文件 | `{DESIGN_DIR}/architecture.md` |
| 注册表 | `element-registry` 中 `parent_element_id == architecture` |
...
| `element_ids[]` | ✅ | `arch-tech-stack`, `arch-deployment`, `arch-microservice`, `arch-feature-toggle`, `arch-code-structure`, `arch-common-lib`, `arch-common-component` |
...
| 0.5 | 模式策略：`build` 按常规执行；`modify` 仅改 `modify_focus` 命中分节；`incremental` 仅当 `impact_analysis` 明确命中架构类变化（如 `AA-*`、跨域架构调整、部署/拓扑变化）时执行，否则整体跳过并给出原因 |
...
- 按**所有适用子要素**的 `prd_sources.required_sections` / `optional_sections` 合集，Read `PRD_FILE` 对应章节，并按 `extraction_keys` 建立架构设计溯源候选
...
|--------|-----------|---------------|----------------|
| `arch-tech-stack` | `spec/architecture/arch-tech-stack.md` | `standards/architecture/arch-tech-stack.md` | 见 element-registry |
| `arch-deployment` | `spec/architecture/arch-deployment.md` | `standards/architecture/arch-deployment.md` | 见 element-registry |
| `arch-microservice` | `spec/architecture/arch-microservice.md` | `standards/architecture/arch-microservice.md` | 见 element-registry |
| `arch-feature-toggle` | `spec/architecture/arch-feature-toggle.md` | `standards/architecture/arch-feature-toggle.md` | 见 element-registry |
| `arch-code-structure` | `spec/architecture/arch-code-structure.md` | `standards/architecture/arch-code-structure.md` | 见 element-registry |
| `arch-common-lib` | `spec/architecture/arch-common-lib.md` | `standards/architecture/arch-common-lib.md` | 见 element-registry |
| `arch-common-component` | `spec/architecture/arch-common-component.md` | `standards/architecture/arch-common-component.md` | 见 element-registry |


## 附录 A：incremental 义务

- artifact 分节内须含 `<!-- DELTA: change=..., chapter=architecture, op=..., level=... -->`
- 每条 DELTA 关联至少一个 DIP（字段同 SKILL.md DesignImpactPoint）

## §7 领域动作单位摘要

- 技术选型：以“技术决策项”为动作单位
- 部署/微服务：以“节点、服务边界、依赖关系”为动作单位
- 代码结构/公共库/公共组件：以“目录或能力条目”为动作单位
- incremental 下优先判断是否属于架构层变化，再决定 `✨新增 / 🔧修改 / ⏭️跳过`
