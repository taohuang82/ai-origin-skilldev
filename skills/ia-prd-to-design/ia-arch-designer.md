...
description: |
  API Contract Subagent — 产出 backend-api.md
domain: backend
...
|------|-----|
| TDD 章节 | 第3章 API 契约 |
| 落盘文件 | `{DESIGN_DIR}/backend-api.md` |
| 注册表 | `element-registry` 中 `parent_element_id == api-contract` |
...
| `PRD_FILE` | ✅ | |
| `prd_sources` | 条件 | orchestration 可透传 `be-api` 的 `element-registry.yaml.prd_sources`；缺失时 Subagent 必须自行读取 registry 解析 |
| `output_doc_path` | ✅ | |
...
| 子步 | 动作 |
|------|------|
| 0.1 | 从 Prompt 获取 `element_ids[]`：`be-api` |
| 0.2 | 读取 `be-api.prd_sources`，基于 `required_sections` / `optional_sections` / `extraction_keys` 判定**本次适用**；不适用标记 `⏭️ 跳过` |
| 0.3 | 若 PRD 无 API/接口诉求 → 全部跳过，返回 `⏭️ 已评估，本次不适用` + 汇总块 |
| 0.4 | 输出**适用子要素序列**（仅列 ID，含跳过原因） |
...
- 按 `be-api.prd_sources.required_sections` / `optional_sections` Read `PRD_FILE` 对应章节，并按 `extraction_keys` 抽取 TC/FR、接口动作、请求/响应字段与权限点
- Read `force_read[]` 中 `{DESIGN_DIR}/data.md` + 其他上游 artifact
...
| 2.4 **落盘** | 参照 standards `## 输出骨架`，写入 `output_doc_path` 对应分节 |
| 2.5 **要素自检** | **在规范尚在上下文时立即检查当前要素**：<br> ① 对照 spec `## 质量检查点` → 逐项核对<br> ② 对照 standards 格式/命名类 MUST 条文 → 逐项核对<br> ③ 空内容检查：表格无数据行 / 章节正文为空<br> ④ 占位符检查：`[待补充]` / `TODO` / `XXX` / `待确认` / `示例值`<br> ⑤ 记录自检结果（✅/❌ + 问题描述），汇总到 `executed_sub_elements[]` |
| 2.6 `[交互]` | 若 spec 含 `[交互]` 步骤：**禁止**向用户提问，自行基于现有上下文做出合理设计决策，标注为"自动决策"后继续当前要素 |

### Step 4：汇总质量自检（轻量，不重新加载规范）

> **逐要素详细自检已在 Step 2.5 完成**，本步骤仅做汇总性校验。

**必须**逐项输出检查结果（未通过的标 ❌），不得仅写「已通过」：

1. **汇总各要素自检结果**：将 Step 2.5 中每个适用子要素的 ✅/❌ 汇总输出
2. **incremental 模式**：确认所有 DELTA 块 + DIP 字段完整
3. **PRD 溯源检查**：接口清单与详细设计中的 `be-api.prd_sources.extraction_keys` 命中值（TC/FR 或 PRD 小节标题等）非空且前后一致；若 PRD 原文缺失，须在返回载荷说明缺失项，不得臆造
4. **artifact 终版检查**：Read `output_doc_path` 全文，检查是否有残留占位符或空章节
...
```text
## 质量自检报告
- [✅] be-api spec 第1项：所有 API 均含请求/响应定义
- [❌] be-api spec 第2项：API-03 缺少错误码映射
  → 已补充（见 artifact §3.1 API-03）
```
...
|--------|-----------|---------------|----------------|
| `be-api` | `spec/backend/be-api.md` | `standards/backend/be-api.md` | 见 element-registry |


## 附录 A：incremental 义务

- artifact 分节内须含 `<!-- DELTA: change=..., chapter=api-contract, op=..., level=... -->`
