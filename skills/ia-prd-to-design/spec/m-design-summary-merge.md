...
在 **`effective_sequence` 中全部计划要素均已通过 Subagent 成功结束**（或本轮编排认定要素交付已就绪）之后，生成 **`{DESIGN_DIR}/design.md`**：**只保留摘要与引用**，不重复粘贴各要素交付文件中的实现细节。
...
| 条件 | 行为 |
|------|------|
| 本轮有计划要素且已全部完成 | **必须执行**汇总（除非用户明确取消特性级 `design.md` 且团队另有约定） |
| 本轮无任何要素派发（极端空跑） | 可跳过；直接覆盖已有 `{DESIGN_DIR}/design.md` |
...
1. **`arch_decisions`（架构决策）**  
   ...
   - 二者皆有则以 **overall-design 为叙述主轴**，shared-context 作补充且不与之冲突；冲突时以 overall-design 为准，记录差异日志。
...
4. **各设计域摘要与引用**  
   - **优先**来自 orchestration 本轮维护的 **`collected_summaries[]`**（由 Subagent Task 返回 → orchestration 轻量后置校验 → 透传 append）。  
...
每个 **`element_id` 经 Subagent 成功结束**后，Subagent Task 须返回下列结构块；orchestration 轻量后置校验通过后 **透传** append 至 `collected_summaries[]`。**标题字面量、字段名与子项顺序不得改动**。  
编排**禁止**代写或补全汇总块；Phase 2A 只 merge `collected_summaries[]`。
...
- `references` 每条须可定位到**真实存在的章节标题**（方案 B：填 `## §x.y …` 字面量，与 `element-registry.output.heading` 一致）；若本次评估为无变更，仍须至少保留一条指向交付文件或摘要结论的引用说明。
...
|------------------------|----------------|
| architecture | `## 顶层架构与全局规范（build mode only）` |
| data-model | `## 数据存储与模型` |
| api-contract | `## API 契约` |
| backend-impl | `## 后端处理逻辑` |
...
- **`EXECUTION_MODE ∈ {incremental, modify}`** 且本轮未执行 `architecture` 时：省略模板中「顶层架构与全局规范」整节，或保留一节并写「本轮未变更架构文档（参见累积视图或基线）」——二选一须在团队内统一；默认 **省略该节**。
...
1. **收集**：编排必须先 Read 本轮 `collected_summaries[]` 中每个已执行要素的 **`## 汇总输入（供 design.md 合并）`**；缺失则不得进入写盘步骤。
   - `design.md` 只合并 `## 汇总输入（供 design.md 合并）`，不得合并 `## 质量自检报告`。
...
若门禁失败：**记录证据路径到日志**，自动回补汇总块后继续汇总；连续失败 3 次则跳过该域并标注"未通过门禁"。
...
**审阅**：自动确认并进入 Phase 3B，无需用户交互。

**约束**：必须完整列出所有 DIP，禁止因数量多而截断。
