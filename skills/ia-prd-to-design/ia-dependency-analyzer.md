...
name: ia-db-designer
description: |
  Data Model Subagent — 产出 data.md
domain: data
...
|------|-----|
| TDD 章节 | 第2章 数据存储 |
| 落盘文件 | `{DESIGN_DIR}/data.md` |
| 注册表 | `element-registry` 中 `parent_element_id == data-model` |
...
| `element_ids[]` | ✅ | `data-table`, `data-cache`, `data-state-machine` |
...
| `output_doc_path` | ✅ | |
| `OVERALL_DESIGN_FILE` | 条件 | 存在则必读（通常为 `{DESIGN_DIR}/overall-design.md`）；否则 `shared-context.md` + PRD |
...
| 子步 | 动作 |
|------|------|
| 0.1 | 从 Prompt 获取 `element_ids[]`：`data-table`、`data-cache`、`data-state-machine` |
...
- 按**所有适用子要素**的 `prd_sources.required_sections` / `optional_sections` 合集，Read `PRD_FILE` 对应章节，并按 `extraction_keys` 建立数据设计溯源候选
...
2. **跨要素一致性**：检查不同要素产出之间是否存在矛盾（如表设计与缓存 Key 不一致）
...
|--------|-----------|---------------|----------------|
| `data-table` | `spec/data/data-table.md` | `standards/data/data-table.md` | 见 element-registry |
| `data-cache` | `spec/data/data-cache.md` | `standards/data/data-cache.md` | 见 element-registry |
| `data-state-machine` | `spec/data/data-state-machine.md` | `standards/data/data-state-machine.md` | 见 element-registry |


## 附录 A：incremental 义务

- artifact 分节内须含 `<!-- DELTA: change=..., chapter=data-model, op=..., level=... -->`
- 每条 DELTA 关联至少一个 DIP（字段同 SKILL.md DesignImpactPoint）

## §7 领域动作单位摘要

- 数据表：以“表/字段/索引”条目为动作单位
- 缓存：以“缓存键/失效策略”条目为动作单位
- 状态机：以“状态节点/流转边”条目为动作单位
- 每项均按 `✨新增 / 🔧修改 / ⏭️跳过` 记录判定依据
