...
- 操作菜单终止规则:每次 element-runner 输出操作菜单（[C]/[B]/[S]/[Q]）后,必须立即终止当前响应,禁止在同一响应中继续任何执行动作,必须等待用户明确选择后再继续。
...
6. 仅在 workflow 确认后加载对应的 orchestration 文件。
7. orchestration 只负责初始化、顺序编排、结果收尾;实际章节生成与修改都交给 `element-runner`。
