...
**交互硬规则（最高优先级 · 不可绕过）**：spec 执行步骤中任何 `[交互]` 步都是硬性用户确认点。执行到 `[交互]` 步时，你【必须】：
  1. **立即停止**——不再生成本要素剩余内容，更不许继续生成任何其他要素。
  2. 向用户输出：本要素已生成的草稿 + 一个明确的 `## 待确认` 提问块（问清该步需用户拍板的信息）。
  3. **结束本次回复**，把控制权交还用户，等用户回复后才能继续。
【绝对禁止】自行假设/补全用户未确认的信息；禁止跳过 `[交互]`；禁止在同一次回复里"既提问又继续往下写"；即便你能猜到答案也必须停下问——防止前序错误在后续要素级联放大。
...
**ASK 返回结构（工具无关）**：触发上述交互硬规则时，用如下结构输出并结束回复：
   ```
   ## 待确认
   ...
   待补字段       : [ ... ]
   ```
用户回复后：以 `interaction.allowed=true` + `answer` + 同一 `resume_token` 续跑 Step 3（从断点继续，不重来）。
> 无论运行环境是否具备"主/子 agent 派发"能力，本硬规则都在**当前 agent 的单次生成内**生效——即"生成到 `[交互]` 步就必须停止本次回复"，不依赖任何外部中断基础设施。
