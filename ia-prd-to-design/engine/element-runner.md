...
- `context`         : Context Box，必须包含：
  ...
  - `impact_analysis`    : {         # incremental 模式下使用
      prd_change_register  : [],    # PC-xx 列表
      triggered_changes    : [],    # AtomicChange 列表
      effective_sequence   : [],    # 路由结果
      element_changes      : [],    # 本要素相关的变化点子集
      impact_points        : []     # DIP 累积（跨要素全局）
    }
  - `change_type`        : ""
...
> **字段说明**：
> - `change`：触发本段增量的原子变化点 ID（来自 atomic-change-registry，如 IA-01）
> - `chapter`：受影响要素 ID
> - `op`：操作类型（add/modify/delete）
> - `level`：影响置信度（certain/likely/conditional）

**DIP 生成协议**（增量设计专属）：
- 每个 DELTA 块必须关联至少一个 DIP
- DIP 字段与 SKILL.md 中 DesignImpactPoint 结构体对齐
- DIP id 在本要素内临时编号，Phase 3A 汇总时全局重编号
- 每条 DIP 必须包含 boundary_constraints（若有禁止修改的对象）
- target_state_evidence 必须标注四档之一（incr_prd / baseline_design / legacy_system / dialog）

**基线对照协议**：
- 读取 context.base_doc_path 中本章节对应内容作为 baseline_state
- 基线缺少对应章节 → 标注 baseline_state = "基线无对应章节"
- 基线内容与增量 PRD 矛盾 → 触发暂停
...
**全局寄存字段写入约束**：
- `prd_change_register`、`triggered_changes`、`impact_points` 等跨要素全局字段
  禁止在单个要素的 Phase 6 写入。这些字段由编排在 Phase 3A 统一写入 design.md。
- 单个要素的 Phase 6 仅更新：
  - 本文件的 `stepsCompleted`、`last_element`、`last_updated`、`status`
  - 本要素特有的 frontmatter 字段（如 requirement_type）