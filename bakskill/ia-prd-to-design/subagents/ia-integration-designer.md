...
description: |
  Integration Subagent — 产出 integration.md
domain: integration
...
|------|-----|
| TDD 章节 | 第6章 系统集成 |
| 落盘文件 | `{DESIGN_DIR}/integration.md` |
| 注册表 | `element-registry` 中 `parent_element_id == integration` |
...
| `element_ids[]` | ✅ | `integration-mq`, `integration-external`, `integration-notification` |
...
- 按**所有适用子要素**的 `prd_sources.required_sections` / `optional_sections` 合集，Read `PRD_FILE` 对应章节，并按 `extraction_keys` 建立集成设计溯源候选
...
2. **跨要素一致性**：检查不同要素产出之间是否存在矛盾（如 MQ 消息体与外部接口参数不一致）
...
|--------|-----------|---------------|----------------|
| `integration-mq` | `spec/integration/integration-mq.md` | `standards/integration/integration-mq.md` | 见 element-registry |
| `integration-external` | `spec/integration/integration-external.md` | `standards/integration/integration-external.md` | 见 element-registry |
| `integration-notification` | `spec/integration/integration-notification.md` | `standards/integration/integration-notification.md` | 见 element-registry |


## 附录 A：incremental 义务

- artifact 分节内须含 `<!-- DELTA: change=..., chapter=integration, op=..., level=... -->`
- 每条 DELTA 关联至少一个 DIP（字段同 SKILL.md DesignImpactPoint）

## §7 领域动作单位摘要

- MQ：以“Topic/事件/消费者组”条目为动作单位
- 外部系统：以“系统对接点/API 契约”条目为动作单位
- 通知：以“通知场景×通道”条目为动作单位
- 优先判断是否是业务通知（integration-notification）或技术告警（通常不入业务设计）
